...
class LLMClient(
    ...
    private var model:String="meta-llama/llama-4-scout-17b-16e-instruct",
) {
    ...
    fun setModel(newModel: String) {
        model = newModel
        DebugLogger.debugLog(TAG, "Model updated to: $model")
    }
    ...
    fun isDefaultConceptMap(jsonString: String): Boolean {
        return try {
            ...
            DebugLogger.errorLog(TAG, "Error checking default concept map: ${e.message}")
            false
        }
    }
}