...
(function () {
  ...
  function solve(r) {
    ...
    if (r.opts.length && r.opts.every(function (o) { return ['<', '=', '>'].indexOf(o.label.trim()) >= 0; })) { var v = vals(r.prompt), a = v[0], b = v[1], an = a < b ? '<' : a > b ? '>' : '='; return { hint: 'Convert both to the same notation - which value is bigger?', hintVoice: 'Which number is bigger?', why: 'Put both numbers in the same units first (e.g. lakhs), then the one with more of those units is larger.', glow: an, submitOnPick: 1, line: an === '=' ? 'They are equal - tap "=".' : 'Tap "' + an + '" - the first number is ' + (a < b ? 'smaller' : 'larger') + '.' }; }
    if (r.opts.length && r.opts.every(function (o) { return /^\d+(\.\d+)?\s*x$/i.test(o.label.trim()); })) { var vv = vals(r.prompt), rr = vv[0] / vv[1], bt = r.opts.reduce(function (m, o) { return Math.abs(parseFloat(o.label) - rr) < Math.abs(parseFloat(m.label) - rr) ? o : m; }); return { hint: 'Roughly how many times bigger is the first number than the second?', hintVoice: 'How many times bigger is it?', why: 'Divide the bigger number by the smaller one to see how many times it fits.', glow: bt.label, submitOnPick: 1, line: 'About ' + rr.toFixed(1) + ' times - tap "' + bt.label + '".' }; }
    if (L.some(function (l) { return /km\/day/i.test(l); }) && r.opts.length) { var d = nums(r.prompt)[0], dy = nums(r.prompt)[1], nd = d / dy, bs = r.opts.reduce(function (m, o) { return Math.abs(nums(o.label)[0] - nd) < Math.abs(nums(m.label)[0] - nd) ? o : m; }); return { hint: 'Distance divided by days gives the daily speed - which option is closest?', hintVoice: 'Distance divided by days - which is closest?', why: 'Speed = distance / days; round it and pick the nearest option.', glow: bs.label, submitOnPick: 1, line: 'Need about ' + Math.round(nd) + ' km/day - tap "' + bs.label + '".' }; }
  ...
  var pill = null, modal = null, curDetail = null;
  function esc(s) { return ('' + s).replace(/[&<>]/g, function (c) { return { '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]; }); }
  function ensurePill() {
    if (pill) return;
    pill = document.createElement('button'); pill.id = '__eduKnowMore'; pill.textContent = 'ⓘ Know more';
    pill.style.cssText = 'position:fixed;right:12px;bottom:82px;z-index:2147483646;background:#4b3fbf;color:#fff;border:none;border-radius:20px;padding:9px 15px;font:600 13px system-ui,sans-serif;box-shadow:0 3px 12px rgba(0,0,0,.35);cursor:pointer';
  ...
  function showModal() {
    ...
    if (!modal) {
      ...
      card.style.cssText = 'background:#fff;color:#101433;max-width:540px;width:100%;max-height:82vh;overflow:auto;border-radius:16px;padding:18px 22px 24px;box-shadow:0 14px 44px rgba(0,0,0,.45);font:400 16px/1.55 system-ui,sans-serif';
      var close = document.createElement('button'); close.textContent = '×';
      close.style.cssText = 'position:sticky;top:0;float:right;background:none;border:none;font-size:28px;line-height:1;color:#999;cursor:pointer';
      close.onclick = function () { modal.style.display = 'none'; };
      var body = document.createElement('div'); body.id = '__eduModalBody';
      card.appendChild(close); card.appendChild(body); modal.appendChild(card);
      modal.onclick = function (e) { if (e.target === modal) modal.style.display = 'none'; };
      document.body.appendChild(modal);
    }
    document.getElementById('__eduModalBody').innerHTML = curDetail; modal.style.display = 'flex';
  }
  function hideKnowMore() { if (pill) pill.style.display = 'none'; if (modal) modal.style.display = 'none'; }
  function setDetail(o) {
    var d = o.detail || o.why;
    if (!d && !o.line) { hideKnowMore(); curDetail = null; return; }
    ...
    if (d) p.push('<p style="margin:0 0 4px"><b>Why &amp; how:</b> ' + esc(d) + '</p>');
    curDetail = p.join('');
